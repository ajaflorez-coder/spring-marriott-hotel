...
public class ClienteController {
	...
	@GetMapping("/info/{id}")
	public String infoCliente(@PathVariable("id") int id, Model model , Model model1) {
		try {
			Optional<Cliente> cliente = clienteService.findById(id);
			if(cliente.isPresent()) {
	...
	public String nuevoAquilerdelaHabitacion(@PathVariable("id") int id, Model model) {
	...
	public String saveAlquilerdelaHabitacion(@ModelAttribute("alquiler") Alquiler alquiler, Model model, SessionStatus status) {
		try {
